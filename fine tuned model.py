...
projection_layer = nn.Linear(512, 384)  # Project from 512 (CLIP) to 384 (SBERT)
...
def train_clip_on_sbert(texts, epochs=10, lr=1e-5, batch_size=16, save_path="clip-finetuned-sbert"):
    ...
    for epoch in range(epochs):
        ...
        for batch_texts, batch_targets in tqdm(dataloader, desc=f"Training Epoch"):
            ...
            targets = torch.ones(outputs.size(0))
...
#     texts = data['text'].tolist()
#     # Train the CLIP model on SBERT embeddings
#     trained_model = train_clip_on_sbert(texts, epochs=10, lr=1e-5, batch_size=16, save_path="clip-finetuned-sbert")
